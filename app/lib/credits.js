...
async function sendEmail({ to, subject, text, html }) {
  ...
  const res = await fetch('https://api.sendgrid.com/v3/mail/send', {
    ...
    body: JSON.stringify({
      ...
      from: { email: fromEmail, name: 'Freelance Profile Toolkit' },
      subject,
      content: [
...
export async function sendVerificationCode(email) {
  ...
  try {
    await sendEmail({
      to: normalized,
      subject: 'Your Freelance Profile Toolkit verification code',
...
export async function sendAccessGrantedEmail(email) {
  ...
  try {
    await sendEmail({
      to: normalized,
      subject: 'Your Freelance Profile Toolkit access is now unlimited',
      text: `Good news — your Freelance Profile Toolkit access is now unlimited. No more free-credit limit on any tool.\n\nRun a check any time: https://profile-rewriter.vercel.app`,
      html: `<p>Good news — your Freelance Profile Toolkit access is now <strong>unlimited</strong>. No more free-credit limit on any tool.</p><p>Run a check any time: <a href="https://profile-rewriter.vercel.app">https://profile-rewriter.vercel.app</a></p>`,
    });
    return { sent: true };
  } catch (err) {
